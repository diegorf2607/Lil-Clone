...
export default function AdminPage({ initialView }: { initialView?: AdminView }) {
  ...
  const visibleReservations = useMemo(() => {
    if (!user) return []
    if (user.role === "staff") {
      if (staffReservations.length > 0) return staffReservations
      return reservations.length > 0 ? reservations : derivedReservations
    }
    ...
    // Admin y Recepcionista: todas las reservas
    return reservations.length > 0 ? reservations : derivedReservations
  }, [reservations, user, staffReservations, derivedReservations])
